...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        subcount = count - sum([1 for n in neighbors if n in self.mines]) 
        neighbors = [n for n in neighbors if n not in self.safes and n not in self.mines] # safe cells can be removed
        if Sentence(neighbors, subcount) not in self.knowledge: self.knowledge.append(Sentence(neighbors, subcount)) #3)

        for sen1 in self.knowledge: #5)
            cells1 = sen1.cells
            count1 = sen1.count
            if len(cells1) == 0: continue
            for sen2 in self.knowledge: 
                if sen1 == sen2: continue
                cells2 = sen2.cells
                count2 = sen2.count
                if len(cells2) == 0: continue
                elif cells2.issubset(cells1):
                    tempcount = count1 - count2
                    tempcells = copy.deepcopy(cells1)
                    tempcells = [cell for cell in tempcells if cell not in cells2 and cell not in self.moves_made]
                    tempsen = Sentence(tempcells, tempcount) 
    ...
    def make_random_move(self):
        ...
        moves = [cell for cell in cells if cell not in self.mines and cell not in self.moves_made]
